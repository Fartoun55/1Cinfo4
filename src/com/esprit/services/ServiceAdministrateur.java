...
public class ServiceAdministrateur {
    ...
    public void modifier(User u) {
        if (u instanceof Administrateur){
            ...
            pst.executeUpdate();
            System.out.println("Administrateur modifiée !");
        } catch (SQLException ex) {
            System.out.println(ex.getMessage());
        }
        }
    ...
    public List<User> afficher(String role) {
        ...
        try {
            ...
            if (role.equals(role)){
            list = new ArrayList<Administrateur>(); 
           while(rs.next()) {
                list.add(new Administrateur(rs.getInt("id_user"), rs.getString("nom"), rs.getString("prenom"), rs.getDate("dateNaissance"), rs.getString("mail"), rs.getInt("tel"), rs.getString("login"),rs.getString("password")));
